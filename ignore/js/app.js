...
import { EditorTab } from "./ui/editor-tab.js";
import { createRoot } from "react-dom/client";
import { SimpAnalysisTab } from "./simp-analysis/SimpAnalysisTab.jsx";
...
const APP_VERSION = "PCF CONVERTER V5.6b";
...
async function boot() {
  try {
    ...
    initConfigTab();
    initViewerTab();
    new EditorTab(); // 3D Isometric Editor
    
    // Simp Analysis Tab (React Mount)
    const simpTabRoot = document.getElementById('simp-analysis_3D-tab');
    if (simpTabRoot) {
      createRoot(simpTabRoot).render(React.createElement(SimpAnalysisTab));
    }
