...
export function runAllBenchmarks() {
  console.log("=== BENCHMARK 1: L-Shape (No SIF) ===");
  {
    ...
    assertClose(F1, 2937, 0.5, "F1");
    assertClose(M1, 176220, 0.5, "M1");
    assertClose(Sb1, 20732, 0.5, "Sb1");

    const SA = allowableStress(f, Sc, Sh);
    assertClose(SA, 30000, 0, "SA"); // Exact check

    const { ratio, result } = stressCheck(Sb1, SA);
    assertClose(ratio, 0.691, 0.5, "Ratio"); // Note: ~0.691 = 20732 / 30000 = 0.691066
  ...
  console.log("=== BENCHMARK 2: Z-Shape (No SIF) ===");
  {
    ...
    assertClose(F2, 5590, 0.5, "F2");
    assertClose(Sb2, 29929, 0.5, "Sb2");

    // L1 absorbing delta_Y
    const { F_lbf: F1, Sb_psi: Sb1 } = gcBasic(E, I, Z, D_o, delta_Y, L1_X);
    assertClose(F1, 431, 0.5, "F1");
    assertClose(Sb1, 3846, 0.5, "Sb1");

    const Sb_E1 = combineStressAtNode([Sb1, Sb2]);
    assertClose(Sb_E1, 30175, 0.5, "Sb_E1");

    const SA = allowableStress(1.0, Sc, Sh);
    const { ratio, result } = stressCheck(Sb_E1, SA);
    assertClose(ratio, 1.006, 0.5, "Ratio");
    if (result !== 'FAIL') throw new Error("BENCHMARK FAIL: Benchmark 2 Result should be FAIL");
  }
  ...
  console.log("=== BENCHMARK 3: Elbow + Tee (With SIF) ===");
  {
    ...
    assertClose(SE_elbow_modified, 9867, 1.0, "SE_elbow_modified");
    const { ratio: ratio_elbow_modified } = stressCheck(SE_elbow_modified, SA);
    assertClose(ratio_elbow_modified, 0.330, 1.5, "Ratio_elbow_modified"); // 9867 / 29850 = 0.33055
    ...
    assertClose(SE_tee, 11216, 0.5, "SE_tee");
    const { ratio: ratio_tee, result: result_tee } = stressCheck(SE_tee, SA);
    assertClose(ratio_tee, 0.376, 0.5, "Ratio_tee");
